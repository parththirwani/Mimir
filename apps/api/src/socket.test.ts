...
const { initSocket, emitToUser } = await import("./socket.js");
...
describe("socket registry", () => {
  ...
  test("emit reaches every of a user's sockets, never others, and evicts on disconnect", async () => {
    const tokenA = await signAccessToken("user-1", "socket-test-secret");
    const [a, b] = await Promise.all([connect(tokenA), connect(tokenA)]);
    const other = await connect(await signAccessToken("user-2", "socket-test-secret"));
