...
const { initPubSub, initSocket } = await import("../infra/socket.js");
const { signAccessToken } = await import("../auth/tokens.js");
const { getConfig } = await import("@mimir/backend-core");
// Sign with the process-cached config secret (the socket middleware verifies
// against it), not a hardcoded one — otherwise the full-suite run rejects the
// handshake because an earlier test file already cached the config.
...
describe("pub/sub -> socket", () => {
  test("worker publish on user-events:{userId} reaches that user's socket on the payload's event name", async () => {
    const token = await signAccessToken("user-1", JWT_SECRET);
