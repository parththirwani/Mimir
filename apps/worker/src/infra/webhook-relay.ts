...
// Webhook processing: the api only INSERTs verified WebhookEvents; this relay
// (mirror of the outbox pattern) polls unprocessed rows, fans each out to the
// matching ACTIVE agents by entity (one webhook, many agents), then marks
// processedAt. Polling (not a stream) is what keeps it crash-safe: a Redis/worker
// outage loses nothing — the row stays unprocessed until the next tick. The
// agent's own run re-fetches live data + filter-before-surfacing decides whether
// the change surfaces, so a wake is cheap and rarely spammy.
